  ...
  try {
    ...
    if (!userLatitude && !userLongitude) {
      return res
        ...
        .json({ message: "Please Provide UserLatitude and UserLangtitude" });
    }
    const parkingLocations = await parking
      .find(
        {
          location: {
            $near: {
  ...
  try {
    ...
    if (!ParkingNumber) {
      return res
        ...
        .json({ message: " Please Provide Park Number" });
    }
    ...
    if (!spots) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "no empty Spots Availabel in Parking Place" });
    }
